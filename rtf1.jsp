...
<%
   ...
   String user = session.getAttribute("email").toString();
       ...
       LocalTime currentTime = LocalTime.now();
        ...
        PreparedStatement pst2=co.prepareStatement("insert into fogrequest(user,owner,eheartbeat,etemprature,eox,edisease,t1,t2,finalt,status,status1,eimage,role,timet1) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
	...
	int i= pst2.executeUpdate();
       PreparedStatement pst3=co.prepareStatement("update fogrequest set status='request_sent' where user = '"+user+"'  and owner = '"+owner+"' and edisease = '"+edisease+"'    ");
       pst3.executeUpdate();
       
      if(i>0){
       response.sendRedirect("rtf.jsp?m1=upload_success"); 
      }
      else{
          response.sendRedirect("rtf.jsp?m2=upload_failed"); 
      }
       }
